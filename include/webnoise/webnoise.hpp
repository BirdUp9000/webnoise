...
private:
  static void m_GenerateTerrainMesh(const std::vector<std::pair<Vector2, float>>& data);
  static std::vector<std::pair<Vector2, float>> m_GenerateNoise(unsigned int dim, float scale, float time, float octaves);
  static void m_UpdateDrawFrame();
  static void m_Draw3D();
  static void m_DrawGUI();
